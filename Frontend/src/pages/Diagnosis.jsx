...
function Diagnosis() {
    ...
    const fetchDig = async () => {
        try {
            setLoading(true);
            const res = await fetch('http://localhost:3001/diagnosis', {
    ...
    const save = async () => {
        if (!form.diagnosisTypeName || !form.hospitalID) {
            alert("Diagnosis Name and Hospital ID are required");
            return;
        }

        try {
            if (editIndex !== null) {
                await fetch(`http://localhost:3001/diagnosis/edit/${list[editIndex]._id}`, {
                    method: "PATCH",
                    headers: { 
                ...
                await fetch('http://localhost:3001/diagnosis/add', {
                    method: "POST",
                    headers: { 
                        "Content-Type": "application/json",
                        "Authorization": `Bearer ${token}`
    ...
    const remove = async (id, index) => {
        if (window.confirm("Are you sure you want to delete this diagnosis?")) {
            try {
                await fetch('http://localhost:3001/diagnosis/delete/' + id, {
                    method: "DELETE",
                    headers: { "Authorization": `Bearer ${token}` }
                });
                fetchDig();
            ...
            <div className="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h2 className="fw-bolder mb-0 text-dark">
                        <i className="bi bi-activity text-primary me-2"></i>Diagnosis Master
                    </h2>
                    <p className="text-muted mb-0">Manage diagnosis types and categories.</p>
                </div>
            </div>
                    ...
                    <div className="card shadow-sm border-0 rounded-4 sticky-top" style={{ top: '90px', zIndex: 1 }}>
                        <div className="card-header bg-white border-bottom p-4">
                            <h5 className="fw-bold text-primary mb-0 d-flex align-items-center">
                                ...
                                {editIndex !== null ? "Edit Diagnosis Type" : "Add Diagnosis Type"}
                            </h5>
                        </div>
                        ...
                        <div className="card-body p-4 bg-light bg-opacity-50">
                            <div className="row g-3">
                                ...
                                <div className="col-12">
                                    <label className="form-label fw-semibold text-secondary small mb-1">Diagnosis Name <span className="text-danger">*</span></label>
                                    <input
                                        type="text"
                                        className="form-control rounded-3"
                                        placeholder="e.g. Viral Fever"
                                        name="diagnosisTypeName"
                                        value={form.diagnosisTypeName}
                                        onChange={handleChange}
                                    />
                                </div>
                                ...
                                <div className="col-sm-8">
                                    ...
                                    <input
                                        ...
                                        value={form.diagnosisTypeShortName}
                                        onChange={handleChange}
                                    />
                                </div>
                        ...
                        <div className="card-body p-0">
                            {loading ? (
                                <div className="text-center p-5 text-muted">
                                    ...
                                    <p className="mb-0">Loading diagnoses...</p>
                                </div>
                            ) : list.length === 0 ? (
                                <div className="text-center p-5 text-muted">
                                    ...
                                    <h6 className="fw-bold">No diagnosis types found</h6>
                                    <p className="small mb-0">Add a new type to get started.</p>
                                </div>
                            ) : (
                                    ...
                                    <table className="table table-hover align-middle mb-0">
                                        <thead className="bg-light">
                                            <tr>
                                                ...
                                                <th className="py-3 text-secondary">Diagnosis</th>
                                                <th className="py-3 text-secondary">Short</th>
                                                <th className="py-3 text-secondary">HospID</th>
                                        ...
                                        <tbody className="border-top-0">
                                            {list.map((item, i) => (
                                                <tr key={item._id || i} style={{ transition: 'all 0.2s' }}>
                                                    ...
                                                    <td>
                                                        <div className="fw-bold text-dark mb-1">{item.diagnosisTypeName}</div>
                                                        {item.description && <small className="text-muted d-block text-truncate" style={{ maxWidth: '150px' }}>{item.description}</small>}
                                                    </td>
                                                    <td>
                                                        <span className="badge bg-light text-dark border px-2 py-1">{item.diagnosisTypeShortName || "-"}</span>
                                                    </td>
